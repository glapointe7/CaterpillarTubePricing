...
We include librairies for graphs and tables to display. We also connect to the Caterpillar database for the next queries to execute.
...
We use only the year and month parts of the date since the day in the same month and year will not influance the cost.
```{r echo = FALSE, message = FALSE, warning = FALSE}
query = "SELECT TA.numberOfBends, TAC.totalWeight, TAC.numberOfComponents, TA.materialID,
        ...
        WHERE TAP.cost > 0;"
train <- database$selectFromTable(query)
...
query = "SELECT TA.numberOfBends, TAC.totalWeight, TAC.numberOfComponents, TA.materialID,
        ...
        FROM TubeAssemblyPricing AS TAP
        	...
        	LEFT JOIN TubeAssemblyWeightView AS TAC ON TAC.fkTubeAssembly = TAP.fkTubeAssembly
        WHERE TAP.cost = 0;"
test <- database$selectFromTable(query)
gc()
...
train_data$quote_date <- as.Date(train_data$quote_date)
train_data$quote_date <- year(train_data$quote_date) * 100 + month(train_data$quote_date)
train_data <- cbind(number_bends = train$numberOfBends, train_data)
train_data <- cbind(weight = ifelse(is.na(train$totalWeight), 0.0, train$totalWeight), train_data)
train_data <- cbind(number_components = ifelse(is.na(train$numberOfComponents), 0, train$numberOfComponents), train_data)
train_data <- cbind(material = ifelse(is.na(train$materialID), 0, as.numeric(substring(train$materialID, 4))), train_data)
train_data <- cbind(volume = ifelse(is.na(train$volume), 0.0, train$volume), train_data)
...
test_data$quote_date <- as.Date(test_data$quote_date)
test_data$quote_date <- year(test_data$quote_date) * 100 + month(test_data$quote_date)
test_data <- cbind(number_bends = test$numberOfBends, test_data)
test_data <- cbind(weight = ifelse(is.na(test$totalWeight), 0.0, test$totalWeight), test_data)
test_data <- cbind(number_components = ifelse(is.na(test$numberOfComponents), 0, test$numberOfComponents), test_data)
test_data <- cbind(material = ifelse(is.na(test$materialID), 0, as.numeric(substring(test$materialID, 4))), test_data)
test_data <- cbind(volume = ifelse(is.na(test$volume), 0.0, test$volume), test_data)


## Model Prediction with Random Forest.
library(randomForest)
set.seed(415)
tube.rf <- randomForest(log(cost + 1) ~., data = train_data, nTree = 25, do.trace = 2)
prediction <- exp(predict(tube.rf, newdata = test_data)) - 1
predict_rf_data <- cbind(test_data$id, prediction)
colnames(predict_rf_data) <- c('id', 'cost')
write.csv(predict_rf_data, "Submission.csv", row.names = FALSE)
```