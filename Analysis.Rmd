...
```{r echo = FALSE, message = FALSE, warning = FALSE, results = 'asis'}
query = "SELECT TAP.fkTubeAssembly, TAP.supplierID, TAP.quoteDate, TAP.anualUsage, TAP.minOrderQuantity,
        		TAP.bracketPricing, TAP.quantity, TAP.cost, Q.minQty, Q.maxQty,
        		TA.numberOfBends, TAC.totalWeight, TAC.numberOfComponents, TA.materialID, TA.bendRadius, TA.diameter, TA.wallThickness,
...
set.seed(415)
rf <- randomForest(log(cost + 1) ~., data = train, nTree = 25, do.trace = 2)
prediction <- exp(predict(rf, newdata = test)) - 1
predict_rf_data <- cbind(test$id, prediction)
...
bst.cv <- xgb.cv(data = train_matrix, nfold = 5, nrounds = 10, objective = "reg:linear")

### Training.
# nround <- 1000
# bst <- xgboost(data = train_matrix, label = train_labels, nrounds = nround, objective = "reg:linear")
# 
# model <- xgb.dump(bst, with.stats = T)
# print(model[1:10])
# 
# prediction <- exp(predict(bst, newdata = test_matrix)) - 1
